...
class OptogeneticEpochsTable(TimeIntervals):
    ...
    __columns__ = (
        ...
        {'name': 'timeseries', 'description': 'index into a TimeSeries object', 'index': True, 'class': TimeSeriesReferenceVectorData},
        {'name': 'stimulation_on', 'description': 'Whether optogenetic stimulation was used at any time during this epoch. If False, then all other metadata values should be 0.', 'required': True},
        {'name': 'pulse_length_in_ms', 'description': 'Duration of one pulse, in ms. Use NaN if stimulation was off.', 'required': True},
        {'name': 'period_in_ms', 'description': 'Duration between the starts of two pulses, in ms. Use NaN if stimulation was off.Note that the interpulse interval = `period_ms` - `pulse_length_ms`', 'required': True},
        {'name': 'number_pulses_per_pulse_train', 'description': 'Number of pulses in one pulse train. After this number of pulses, no more stimulation occurs until the next train begins (see `intertrain_interval_ms`). Use -1 if stimulation was off.', 'required': True},
        ...
        {'name': 'optical_fiber_locations_table_region', 'description': 'References row(s) of OpticalFiberLocationsTable.', 'required': True, "table": True}
    )
